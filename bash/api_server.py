#!/usr/bin/env python3
"""
Bash RAG API Server for OpenWebUI Integration
"""
...
app = FastAPI(title="Bash RAG API", version="1.0.0")
...
async def startup_event():
    global rag_pipeline
    logger.info("Initializing Bash RAG pipeline...")
    rag_pipeline = EnhancedRAGSystem(
        technology_name="bash",
        docs_dir="docs",
        pdfs_dir="pdfs", 
        chroma_db_path="vector_db",
        collection_name="bash_docs"
    )
    logger.info("Bash RAG API ready!")

@app.get("/")
async def root():
    return {"message": "Bash RAG API is running", "technology": "Bash"}
