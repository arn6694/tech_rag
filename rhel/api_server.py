#!/usr/bin/env python3
"""
RHEL RAG API Server for OpenWebUI Integration
"""
...
app = FastAPI(title="RHEL RAG API", version="1.0.0")
...
async def startup_event():
    global rag_pipeline
    logger.info("Initializing RHEL RAG pipeline...")
    rag_pipeline = EnhancedRAGSystem(
        technology_name="rhel",
        docs_dir="docs",
        pdfs_dir="pdfs", 
        chroma_db_path="vector_db",
        collection_name="rhel_docs"
    )
    logger.info("RHEL RAG API ready!")

@app.get("/")
async def root():
    return {"message": "RHEL RAG API is running", "technology": "Red Hat Enterprise Linux"}
