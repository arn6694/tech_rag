#!/usr/bin/env python3
"""
PowerShell RAG API Server for OpenWebUI Integration
"""
...
app = FastAPI(title="PowerShell RAG API", version="1.0.0")
...
async def startup_event():
    global rag_pipeline
    logger.info("Initializing PowerShell RAG pipeline...")
    rag_pipeline = EnhancedRAGSystem(
        technology_name="powershell",
        docs_dir="docs",
        pdfs_dir="pdfs", 
        chroma_db_path="vector_db",
        collection_name="powershell_docs"
    )
    logger.info("PowerShell RAG API ready!")

@app.get("/")
async def root():
    return {"message": "PowerShell RAG API is running", "technology": "PowerShell"}
