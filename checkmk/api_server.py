...
from rag_system import CheckmkRAGSystem

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

app = FastAPI(title="Checkmk Knowledge API", version="1.0.0")
rag_system = None
...
async def startup_event():
    global rag_system
    logger.info("Initializing Checkmk RAG system...")
    rag_system = CheckmkRAGSystem()
    logger.info("Checkmk RAG API ready!")

@app.get("/")
async def root():
    return {"message": "Checkmk Knowledge API is running", "technology": "checkmk"}
...
async def query_documents(request: QueryRequest):
    ...
    try:
        ...
        logger.error(f"Error processing Checkmk query: {e}")
        raise HTTPException(status_code=500, detail=f"Error processing query: {str(e)}")

if __name__ == "__main__":
    print("Starting Checkmk API server on port 8002")
    print("Add http://localhost:8002 as a knowledge source in Open WebUI")
